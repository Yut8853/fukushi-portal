...
export default function FeedbackPrompt({ context }: { context: string }) {
  ...
  const answer = (helpful: boolean) => {
    window.dispatchEvent(new CustomEvent("support-feedback", {
    ...
    <section className="feedback-prompt" aria-label="案内へのフィードバック">
      {answered ? (
        ...
        <>
          <p>この案内は役に立ちましたか？</p>
          <div>
            <button type="button" onClick={() => answer(true)}>はい</button>
            <button type="button" onClick={() => answer(false)}>いいえ</button>
          </div>
