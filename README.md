...
同一番号なら、利用者がそのまま読める具体的な台本を持つカードを残します。電話番号が空のカードは公式ページや来所先として意味が異なる場合があるため、電話番号による重複排除の対象外です。表示名も重複する場合は正式名を使い、それでも同名なら`serviceArea`を付けて区別します。
...
- 窓口カードは正規化した電話番号でも重複排除する。同一番号ならカテゴリ直通、具体的な
  取り次ぎ台本、一般代表の順で優先し、同じ場所へ複数回かけさせる表示を作らない
