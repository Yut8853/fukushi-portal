...
| --- | --- |
| 都道府県マスター | 47件 |
| ポータル登録自治体 | 348件（`standard` 48件、`basic` 300件） |
| 全国団体データ | 1,919件（現行自治体1,741件、政令市行政区171件、集計・特殊地域7件） |
| 公式URL確定済み | 1,292 / 1,741自治体 |
| 公式URL未確定 | 449自治体 |
| クロールキュー | 1,292件（`pending` 1,290件） |
| クロール処理済み | 2件（水戸市、つくば市） |
| クロール候補 | 26件、すべて`review_required`（CSV未反映） |
| ローカル承認フロー | 編集・確認・監査・CSV公開・失敗時復元を実装済み |
| 利用者向けUI | 状況→地域→案内の3段階、モバイル優先、選択状態と結果件数を明示 |
| 窓口データ | 211件（`published` 199件、`review_required` 12件） |
...
- 全国一括クロールは開始していません。水戸市とつくば市だけで限定試験を実施しました。
...
```text
総数: 1292 / 処理済み: 2 / 残件: 1290 / 進捗: 0.2%
水戸市: review_required、候補15件
つくば市: partial、候補11件、リンク切れPDF 1件
候補合計: 26件（HTML/PDF、全件review_required）
```
...
- 全国一括クロールは未開始（水戸市・つくば市の限定試験のみ実施）
...
9. 水戸市・つくば市の26候補を管理画面で一次情報と照合し、重複・分類・電話番号を確認する
10. 確認済み候補だけをCSVへ公開し、利用者画面で出典と確認日を目視する
11. 次に茨城県を少数件ずつ試験し、失敗率、取得ページ数、レビュー所要時間を計測する
