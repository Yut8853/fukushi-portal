...
export default async function SourcesPage() {
  ...
  const attention = monitorRecords.filter(
    (record) =>
  ...
  return (
    <main id="main" className="page-shell admin-shell">
      ...
      <h1>出典と変更監視</h1>
      <p>
        公開出典 {data.sources.length}件・要確認 {attention.length}件
      </p>
            ...
            {data.sources.map((source) => {
              ...
              return (
                <tr key={source.id}>
                  ...
                  <td>{source.status}</td>
                  <td>
                    <span className={`status status-${record?.status ?? "unmonitored"}`}>
                      {record?.status ?? "未監視"}
                    </span>
                    {record?.error && <small className="job-error">{record.error}</small>}
                  </td>
                  <td>
                    {record?.checkedAt
                      ? new Date(record.checkedAt).toLocaleString("ja-JP")
                      : "未実施"}
                  </td>
                </tr>
              );
          ...
          <tbody>
            {crawlSources.map((item) => (
              <tr key={item.sourceUrl}>
                ...
                <td>{item.sourcePublishedAt || "不明"}</td>
