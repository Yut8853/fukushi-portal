...
export default async function ReviewPage({ searchParams }: ReviewPageProps) {
  ...
  return (
    <main id="main" className="page-shell admin-shell">
      ...
      <h1>クロール結果レビュー</h1>
      <p>
        ログイン中: <strong>{authenticatedUser}</strong>（{authenticatedRole}）
      </p>
      <p>
        ...
        {candidates.map((item) => {
          ...
          return (
            <article className="result-card" key={`${item.code}-${item.id}`}>
              <p className="eyebrow">
                {item.municipalityName} / 信頼度 {Math.round(item.confidence * 100)}% /{" "}
                {item.status}
              </p>
              ...
              <dl>
                ...
                <dt>取得日時</dt>
                <dd>{item.extractedAt}</dd>
                <dt>確認者</dt>
                <dd>{item.reviewer || "未確認"}</dd>
                <dt>確認日時</dt>
                <dd>{item.reviewedAt || "未確認"}</dd>
                <dt>警告</dt>
                <dd>{item.warnings.join(" / ") || "なし"}</dd>
                {item.publishedEntityId ? (
                  <>
                    <dt>公開ID</dt>
                    <dd>{item.publishedEntityId}</dd>
                  </>
                ) : null}
              </dl>
              <details>
              ...
              <form action={reviewAction} className="review-editor">
                ...
                <fieldset disabled={immutable}>
                  ...
                  <div className="review-field-grid">
                    ...
                    <label>
                      ...
                      <select
                        ...
                        defaultValue={
                          item.reservationRequired === null ? "" : String(item.reservationRequired)
                        }
                      >
                    ...
                    <div className="review-field-grid">
                      ...
                      <label>
                        ...
                        <select
                          ...
                          defaultValue={
                            item.repaymentRequired === null ? "" : String(item.repaymentRequired)
                          }
                        >
                          <option value="">未確認</option>
                      ...
                      <label className="review-wide">
                        書類が不足する場合の案内
                        <input
                          name="documentsOptionalNote"
                          defaultValue={item.documentsOptionalNote}
                        />
                      </label>
                      <label className="review-wide">
                        申請の流れ
                        <textarea
                          ...
                          defaultValue={item.applicationFlow}
                          rows={3}
                        />
                      </label>
                    </div>
                  ...
                  <div className="review-actions">
                    ...
                    <button name="action" value="research">
                      再調査
                    </button>
                  </div>
                </fieldset>
              ...
              {item.status === "verified" ? (
                <form action={reviewAction} className="publish-candidate">
                  ...
                  <input type="hidden" name="id" value={item.id} />
                  <p>
                    この操作はCSVへ追記し、一般画面へ公開します。公開前に上の内容を再確認してください。
                  </p>
                  <button name="action" value="publish" disabled={authenticatedRole !== "admin"}>
                    CSVへ公開する
                  </button>
