...
async function loadPublicPortalData(): Promise<PortalData> {
  ...
  const offices = data.offices.filter(
    (item) =>
      ...
      (item.scope === "national" ||
        (item.scope === "prefecture" && Boolean(item.prefectureCode)) ||
        (item.scope === "municipality" && municipalityIds.has(item.municipalityId))),
  );
  ...
  const municipalityPrograms = data.municipalityPrograms.filter(
    (item) =>
      item.status === "published" &&
      municipalityIds.has(item.municipalityId) &&
