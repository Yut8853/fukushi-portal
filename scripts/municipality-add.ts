...
async function main() {
  ...
  try {
    ...
    if (municipalities.some((item) => item.municipalityCode === municipalityCode)) {
      throw new Error(
        `自治体コード「${municipalityCode}」は既に登録されています。追加しませんでした。`,
      );
    }
    if (
      municipalities.some((item) => item.prefectureCode === prefectureCode && item.name === name)
    ) {
      throw new Error(
        `${prefecture.name}の「${name}」は既に登録されています。追加しませんでした。`,
      );
    }
    const municipalityType = (
      await rl.question("自治体種別（special_ward/city/town/village）: ")
    ).trim();
    ...
    if (
      !municipalityTypeSchema.safeParse(municipalityType).success ||
      !supportLevelSchema.safeParse(supportLevel).success ||
      !result.success
    ) {
    ...
    const columns = [
      result.data.id,
      result.data.prefectureCode,
      result.data.municipalityCode,
      result.data.name,
      result.data.nameKana,
      result.data.municipalityType,
      result.data.officialUrl,
      result.data.representativePhone,
      result.data.supportLevel,
      result.data.status,
      result.data.lastVerifiedAt,
    ];
    await appendFile(
      path.join(dataDir, "municipalities.csv"),
