...
console.log(JSON.stringify({
  ...
  candidates: candidates.length, reviewRequired: candidates.filter((item) => item.status === "review_required").length,
  pdfParsed: jobs.reduce((sum, job) => sum + job.documentsParsed, 0),
  categories: Object.fromEntries(categories),
  errors: jobs.filter((job) => job.lastError).map((job) => ({ code: job.municipalityCode, name: job.municipalityName, error: job.lastError })),
}, null, 2));
}
