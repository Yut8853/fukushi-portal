...
async function main() {
  ...
  const requestedIds = argument("sources").split(",").map((item) => item.trim()).filter(Boolean);
  const limitText = argument("limit");
  const limit = limitText ? Number.parseInt(limitText, 10) : 0;
  if (!all && !requestedIds.length && !limit) {
    throw new Error("安全のため --all、--sources=出典ID,出典ID、または --limit=件数を指定してください。");
  }
  ...
  results.filter((result) => result.status !== "ok").forEach((result) =>
    console.log(`  ${result.sourceId}: ${result.error || "内容変更を検出"}`),
  );
