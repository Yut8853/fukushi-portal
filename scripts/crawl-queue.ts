...
async function main() {
  ...
  if (!all && !prefecture && !codes?.length) {
    throw new Error(
      "--all、--prefecture=08、または --municipalities=082015,082201 を指定してください。",
    );
  }
  ...
  let selected = all
    ...
    : prefecture
      ? municipalities.filter(
          (item) =>
            item.prefectureCode === prefecture &&
  ...
  console.log(
    `キュー追加: ${added}件 / 重複統合: ${duplicatesRemoved}件 / URL未登録で除外: ${missingUrls.length}件 / キュー総数: ${byCode.size}件`,
  );
  if (missingUrls.length)
    console.log(
      `URL未登録例: ${missingUrls
        .slice(0, 10)
        .map((item) => `${item.municipalityCode}:${item.name}`)
        .join(", ")}`,
    );
}
