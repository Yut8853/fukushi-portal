...
async function main() {
  ...
  if (force && !prefecture && municipalityCodes.size === 0 && !args.includes("--all")) {
    throw new Error(
      "--force には --municipalities=自治体コード、--prefecture=都道府県コード、または --all が必要です。",
    );
  }
  ...
  for (const job of jobs) {
    ...
    if (matches && force && job.status !== "running") {
      ...
      const hasHumanReview =
        result?.candidates.some((candidate) => candidate.status !== "review_required") ?? false;
      if (hasHumanReview) {
        console.warn(
          `${job.municipalityCode} ${job.municipalityName}: 人間のレビュー履歴があるため再投入しません。`,
        );
        continue;
      }
      job.status = "pending";
