...
const headers = ["id", "prefectureCode", "municipalityCode", "name", "nameKana", "municipalityType", "entityType", "parentMunicipality", "lifecycleStatus", "officialUrl", "sourceUrl"];
...
function slug(kana: string, type: string): string {
  const endings: Record<string, RegExp> = {
    city: /し$/, special_ward: /く$/, town: /(?:まち|ちょう)$/, village: /(?:むら|そん)$/,
  };
  const base = kana.replace(endings[type] ?? /$^/, "");
  return toRomaji(base).toLowerCase().replace(/[^a-z0-9]/g, "");
}

function candidates(item: CsvRow, prefectureSlug: string): string[] {
  const name = slug(item.nameKana, item.municipalityType);
  const prefix = item.municipalityType === "village" ? "vill"
    : item.municipalityType === "special_ward" ? "city" : item.municipalityType;
  const alternatives = item.municipalityType === "village" ? ["vill", "village"]
    : item.municipalityType === "special_ward" ? ["city", "ward"] : [prefix];
  return [...new Set(alternatives.flatMap((kind) => [
...
async function searchCandidates(item: CsvRow): Promise<string[]> {
  ...
  const urls: string[] = [];
  const headers = { "user-agent": "Mozilla/5.0 (compatible; FukushiPortalOfficialUrlResolver/1.0)" };
  ...
  return [...new Set(urls)].filter((raw) => {
    try {
      ...
      return url.protocol === "https:"
        && !/yahoo|duckduckgo|wikipedia|youtube|facebook|instagram|line\.me|x\.com$/.test(url.hostname);
    } catch {
      return false;
    }
  }).slice(0, 10);
...
function privateAddress(address: string): boolean {
  if (address === "::" || address === "::1" || address.startsWith("127.") || address.startsWith("10.")
    || address.startsWith("192.168.") || address.startsWith("169.254.")) return true;
...
async function resolvePublicUrl(rawUrl: string): Promise<URL> {
  ...
  if (url.protocol !== "https:" || url.username || url.password) throw new Error("安全でない候補URL");
  const addresses = await Promise.race([
    resolve4(url.hostname),
    new Promise<never>((_, reject) => setTimeout(() => reject(new Error("DNS確認タイムアウト")), 3000)),
  ]);
...
async function fetchIdentity(rawUrl: string, municipalityName: string, strictTitle = false, expectedSlug = ""): Promise<string> {
  ...
  for (let redirects = 0; redirects <= 4; redirects += 1) {
    ...
    try {
      ...
      const title = text.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1].replace(/<[^>]+>/g, "") ?? "";
      const governmentHost = current.hostname === "lg.jp" || current.hostname.endsWith(".lg.jp");
      const municipalityHost = !expectedSlug || current.hostname.replace(/[^a-z0-9]/g, "").includes(expectedSlug);
      const identity = text.includes(municipalityName)
...
async function main() {
  ...
  const requestedCodes = new Set((process.env.URL_RESOLVER_CODES ?? "").split(",").filter(Boolean).map((code) => code.slice(0, 5)));
  const targets = rows.filter((item) => item.entityType === "municipality" && !item.officialUrl
    ...
    while (cursor < targets.length) {
      ...
      for (const candidate of candidates(item, prefectureSlug)) {
        try {
          ...
          if (process.env.URL_RESOLVER_DEBUG === "1") {
            console.log(`候補失敗 ${candidate}: ${error instanceof Error ? error.message : String(error)}`);
          }
      ...
      if (!item.officialUrl && process.env.URL_RESOLVER_SEARCH === "1") {
        try {
          ...
          if (process.env.URL_RESOLVER_DEBUG === "1") {
            console.log(`検索失敗 ${item.name}: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
      }
      const done = cursor;
  ...
  await save(rows);
  console.log(`公式URL確定: 新規${found}件 / 登録済み合計${rows.filter((item) => item.entityType === "municipality" && item.officialUrl).length}件`);
}
