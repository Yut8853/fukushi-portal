...
async function main() {
  ...
  const prefecture = args.find((arg) => arg.startsWith("--prefecture="))?.split("=")[1]?.trim() ?? "";
  const requestedCodes = args.find((arg) => arg.startsWith("--municipalities="))
    ?.split("=")[1]
    ?.split(",")
    .map(baseCode)
    .filter(Boolean) ?? [];
  ...
  if (!all && !prefecture && !requestedCodes.length) {
    throw new Error("安全のため対象指定が必要です。--all、--prefecture=08、または--municipalities=082015,082201を指定してください。");
  }
  ...
  let jobs = (await readQueue()).filter((job) =>
