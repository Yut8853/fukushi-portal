...
type Row = {
  id: string; prefectureCode: string; municipalityCode: string; name: string;
  nameKana: string; municipalityType: string; entityType: string;
  parentMunicipality: string; lifecycleStatus: string; officialUrl: string; sourceUrl: string;
};

function checkDigit(code: string): string {
  const weights = [6, 5, 4, 3, 2];
  const remainder = code.split("").reduce((sum, digit, index) => sum + Number(digit) * weights[index], 0) % 11;
...
async function main() {
  ...
  const urls = new Map(existing.map((item) => [item.municipalityCode.slice(0, 5), item.officialUrl]));
  const rows: Row[] = [];
  for (let page = 1; page <= 96; page += 1) {
    const sourceUrl = `https://www.e-stat.go.jp/municipalities/cities/areacode?page=${page}`;
    const response = await fetch(sourceUrl, { headers: { "user-agent": "FukushiPortalMunicipalitySync/1.0" } });
    if (!response.ok) throw new Error(`e-Stat ${page + 1}ページ目: HTTP ${response.status}`);
    const $ = cheerio.load(await response.text());
    const codes = $("table.__fix tbody tr .htCode").map((_, element) => $(element).text().trim()).get();
    ...
    detailRows.each((index, element) => {
      ...
      const entityType = code === "13100"
        ? "aggregate"
        : Number(code) >= 1695 && Number(code) <= 1700
          ? "special_territory"
          : parent.endsWith("市") && self.endsWith("区") ? "administrative_ward" : "municipality";
      const name = self || parent;
      const kana = selfKana || parentKana;
      if (!name) return;
      rows.push({
        id: `jp-${code}`, prefectureCode: code.slice(0, 2), municipalityCode: checkDigit(code),
        name, nameKana: kana, municipalityType: municipalityType(name, entityType),
        entityType, parentMunicipality: entityType === "administrative_ward" ? parent : "",
        lifecycleStatus: "current", officialUrl: urls.get(code) ?? "", sourceUrl,
      });
    });
  ...
  const header = ["id", "prefectureCode", "municipalityCode", "name", "nameKana", "municipalityType", "entityType", "parentMunicipality", "lifecycleStatus", "officialUrl", "sourceUrl"];
  const lines = [header.join(","), ...unique.map((row) => header.map((key) => escapeCsv(row[key as keyof Row])).join(","))];
  await writeFile(path.join(process.cwd(), "data", "nationwide-municipalities.csv"), `${lines.join("\n")}\n`, "utf8");
  ...
  console.log(`登録完了: 現行市区町村 ${municipalityCount} / 政令指定都市行政区 ${wardCount} / 集計・特別地域 ${excludedCount} / 合計 ${unique.length}`);
  console.log(`公式URL登録済み: ${unique.filter((row) => row.officialUrl).length} / 未登録: ${unique.filter((row) => !row.officialUrl).length}`);
}
