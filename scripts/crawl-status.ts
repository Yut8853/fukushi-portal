...
async function main() {
  ...
  const done = jobs.filter((job) =>
    ["completed", "review_required", "partial", "failed", "blocked_by_robots", "skipped"].includes(
      job.status,
    ),
  ).length;
  console.log(
    `総数: ${jobs.length} / 処理済み: ${done} / 残件: ${jobs.length - done} / 進捗: ${jobs.length ? ((done / jobs.length) * 100).toFixed(1) : "0.0"}%`,
  );
